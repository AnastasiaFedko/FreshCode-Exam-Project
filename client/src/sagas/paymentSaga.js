...
import * as paymentController from '../api/rest/paymentController';

export function* paymentSaga(action) {
  yield put({ type: ACTION.PAYMENT_ACTION_REQUEST });
  try {
    yield paymentController.payMent(action.data);
...
export function* cashoutSaga(action) {
  ...
  try {
    const { data } = yield paymentController.cashOut(action.data);
